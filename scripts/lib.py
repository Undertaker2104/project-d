...
trivial_tokens = set(open("../data/trivial_tokens.txt", encoding="utf-8").read().split('\n'))

# tokens die niet in GiGaNT staan, meer dan 3 keer in merged_data, en geen
# onderdeel/machine zijn
ignored_tokens = set(open("../data/ignored_tokens.txt", encoding="utf-8").read().split('\n'))
...
parts_lem = re.split("\t|\n", open("../data/parts_lemmatization.txt", encoding="utf-8").read())
...
actionscopes = _pd.read_excel("../excel_files/merged_data.xlsx", index_col="Code")
